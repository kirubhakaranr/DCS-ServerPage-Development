...
function validateAccount(e){
    ...
    if(!email_re.test(email)){
        return fail("Account email doesn't look valid. Please read DCS instruction before proceeding.");
    }

    // parse account public key
    var pubkey = document.getElementById("pubkey").value;
    if(pubkey === ""){
        return fail("You need to include an Account Public Key. Please read DCS instruction before proceeding.");
    }
    var unarmor = /-----BEGIN PUBLIC KEY-----([A-Za-z0-9+\/=\s]+)-----END PUBLIC KEY-----/;
    if(!unarmor.test(pubkey)){
        return fail("Your public key isn't formatted correctly. Please read DCS instruction before proceeding.");
    }
    ...
    try{
        ...
        return fail("Failed validating RSA public key.  Please read DCS instruction before proceeding.");
    }
    ...
    sha256(new Uint8Array(jwk_bytes), function(hash, err){
        ...
        window.setTimeout(function(){
            ...
            status.className = "";
            status.innerHTML = "";
            status.appendChild(document.createTextNode("Looks good! 20% Completed. Proceed to Step 2!"));
        }, 300);
    });
}
...
function validateCSR(e){
    ...
    if(!(ACCOUNT_EMAIL && ACCOUNT_PUBKEY)){
        return fail("Need to complete Step 1 first. Please read DCS instruction before proceeding.");
    }
